(() => {
  ...
  const createInputValueTemplate = (data) => {
    switch (data.type) {
      case "checkbox":
        return `${createInputTagTemplate(data)} ${createLabelTagTemplate(
          data
        )}`;
      case "radio":
        return createFieldsetTagTemplate(data);
      default:
        return `${createLabelTagTemplate(data)} ${createInputTagTemplate(
          data
        )}`;
    }
  };
  ...
  if (typeof window.SmallForm === "undefined") {
    window.SmallForm = SmallForm;
  } else {
    console.log("Library SmallForm already defined.");
  }
})();
